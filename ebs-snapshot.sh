...
snapshot_volumes() {
	for volume_id in $volume_list; do
		...
		snapshot_id=$(aws ec2 create-image --region $region --output=text --description $snapshot_description --volume-id $volume_id --query SnapshotId)
...
cleanup_snapshots() {
	for volume_id in $volume_list; do
		snapshot_list=$(aws ec2 describe-images --region $region --output=text --filters "Name=volume-id,Values=$volume_id" "Name=tag:CreatedBy,Values=AutomatedBackup" --query Snapshots[].SnapshotId)
