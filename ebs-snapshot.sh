...
deleteAMI() {
                ...
                teste = $(aws ec2 describe-images --region $region --filters Name=name,Values="$instance_id"_"$(date +%d%b%y --date ''$retention_days' days ago')" --query 'Images[*].BlockDeviceMappings[*].Ebs.SnapshotId' --output text)
                echo $teste
                echo "teste2"
